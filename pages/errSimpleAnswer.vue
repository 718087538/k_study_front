...
export default {
  ...
  async asyncData({ params, query }) {
    ...
    let { data } = await axios.get(
      `http://106.53.238.187:8003/api/client/simpleAnswer?userId=${query.userId}&categoryId=${query.categoryId}&chapterId=${query.chapterId}&err=1`
    );
    console.log(data);
  ...
  methods: {
    ...
    async next() {
      let { data } = await axios.get(
        `http://106.53.238.187:8003/api/client/simpleAnswer?userId=${this.uid}&categoryId=${this.simpleAnswer.categoryId}&chapterId=${this.simpleAnswer.chapterId}&errNext=1&questionId=${this.simpleAnswer._id}`
      );
      this.showKey = false;
      this.myKey = " ";
      ...
      if (data.code === 206) {
        ...
        this.simpleAnswer = data.data;
      }
    },
    async pre() {
      let { data } = await axios.get(
        `http://106.53.238.187:8003/api/client/simpleAnswer?userId=${this.uid}&categoryId=${this.simpleAnswer.categoryId}&chapterId=${this.simpleAnswer.chapterId}&errNext=0&questionId=${this.simpleAnswer._id}`
      );
      this.showKey = false;
      this.myKey = " ";
      if (data.code === 206) {
